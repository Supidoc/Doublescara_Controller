...
status_t TMC_Write_GCONF( TMC_SerialAddress_t serialAdress, TMC_GCONF_t *gconf){
    ...
    TMC_write(serialAdress, TMC_GCONF_ADDR, &data);
}
...
status_t TMC_read(TMC_SerialAddress_t serialAdress, uint8_t regAddr, uint32_t * data)
{
    ...
    status = UART_RTOS_Send(&UART1_rtos_handle, transmit_read_package,
    TMC_READ_PACKAGE_SIZE);
    ...
    *data = transmit_response_package[3] << 24 | transmit_response_package[4] << 16 | transmit_response_package[5] << 8 | transmit_response_package[6];
