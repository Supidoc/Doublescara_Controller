...
#include "step_shared.h"
     ...
     * task-safe communication. It also initialzes the timer and gpio peripheral for step
     * generation. It must be called before using any other functions in this module.
     *
     ...
     *
     * This function populates a STP_StepperConfig_t structure with default values for initializing
     * a stepper motor control instance.
     *
     ...
     */
    status_t STP_move_relative_prepare_async(STP_Handle_t handle, int32_t steps,
    ...
    /**
     * @brief Sets the tracked absolute step position of a motor.
     *
     * This function updates the software-maintained absolute position counter
     * for the selected stepper handle.
     *
     * @param[in] handle Stepper motor handle to update.
     * @param[in] absoluteSteps New absolute position in steps.
     *
     * @return kStatus_Success if the value is applied.
     *         kStatus_Fail if parameters are invalid.
     */
     ...
     */
    status_t STP_set_acceleration_async(STP_Handle_t handle, double acceleration,
                                        TickType_t deadline, CHD_CmdHandle_t* cmdHandle);
     ...
     *         kStatus_Fail if parameters are invalid, queueing fails, or deadline expires.
     */
    status_t STP_set_end_velocity_async(STP_Handle_t handle, double endVelocity,
                                        TickType_t deadline, CHD_CmdHandle_t* cmdHandle);
