 ...
 *     Includes    *
 ********************/
#include "scara_kinematics_process.h"
#include "scara_kinematics_internal.h"
...
static status_t SKi_handle_change_side(TickType_t deadline)
{
    ...
    MTR_move_absolute_angle_async(skRightMotorHandle, theta1Deg, deadline, &moveCmdHandle);
    if (SKi_wait_and_release(moveCmdHandle, deadline) != kStatus_Success)
    {
        return kStatus_Fail;
    }
