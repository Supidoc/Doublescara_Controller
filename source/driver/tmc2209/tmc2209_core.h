...
#include "peripherals.h"
     ...
     *
     * @note This function is NOT task-safe and must be called during system initialization before
     * any tasks start.
     ...
     *
     * This function populates a TMC_Config_t structure with default values for initializing a
     * TMC2209 driver handle.
     *
     * @param[out] config Pointer to a TMC_Config_t structure to be filled with default
     * configuration values. Must not be NULL.
     *
     * @return kStatus_Success if the default configuration was successfully retrieved and stored in
     * the provided structure. kStatus_Fail if the provided pointer is NULL or an error occurs while
     * retrieving
     */
     ...
     * @param[out] handle Pointer to a TMC_Handle_t variable where the found handle will be stored.
     * Must not be NULL.
     *
     * @return kStatus_Success if a handle with the specified label is found and returned.
     *         kStatus_Fail if the label is NULL, the handle pointer is NULL, or no matching handle
     * is found.
     */
     ...
     *
     * @return kStatus_Success if the command is successfully queued.
     *         kStatus_Fail if the queue is full, handle is NULL, cmdHandle is NULL, or an error
     * occurs.
     *
     ...
     *
     * This function converts a TMC_MICROSTEPPING_t value into the corresponding numeric
     * microstepping resolution used by higher-level configuration code.
     *
     ...
     *
     * The result is rounded according to the specified rounding mode to ensure valid divider
     * values. Dividers are limited to the valid range [0, 31] as per TMC2209 register bit width.
     *
