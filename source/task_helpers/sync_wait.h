/************************************************************
 * @file    sync_wait.h
 * @brief   Synchronous wait helpers for command completion.
 * @author  dg
 * @date    13 Apr 2026
 ************************************************************/
...
#include "cmd_handle.h"
     ...
     */
    status_t SYW_cmd_wait_result(CHD_CmdHandle_t cmdHandle, TickType_t deadline,
                                 EventBits_t* outBits);
     ...
     *                                Can be NULL.
     * @param[out] outBits Optional pointer to receive result bits (CHD_CMD_BIT_SUCCESS,
     *                      CHD_CMD_BIT_FAILURE, CHD_CMD_BIT_TIMEOUT) of the first completed
     * command. Can be NULL.
     *
     * @return kStatus_Success if any command has CHD_CMD_BIT_SUCCESS bit set.
     *         kStatus_Fail if the first completed command has CHD_CMD_BIT_FAILURE bit set.
